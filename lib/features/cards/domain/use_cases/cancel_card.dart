...
class CancelCard implements UseCase<TagCardEntity, CancelCardParams> {
  const CancelCard({
  ...
  @override
  Future<TagCardEntity> call(CancelCardParams params) async {
    final result = await _cardRepository.cancelCard(params.cardId);
...
class CancelCardParams extends Equatable {
  const CancelCardParams({required this.cardId});
