...
class CompleteCard implements UseCase<TagCardEntity, CompleteCardParams> {
  const CompleteCard({
  ...
  @override
  Future<TagCardEntity> call(CompleteCardParams params) async {
    final result = await _cardRepository.completeCard(params.cardId);
...
class CompleteCardParams extends Equatable {
  const CompleteCardParams({required this.cardId});
